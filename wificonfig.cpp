...
bool loadConfigData()
{
  ...
  if (file)
  {
    ...
    file.readBytes(redisIP, 15);
    file.readBytes(redisPW, 15);
    file.readBytes(redisModule, 3);
...
void saveConfigData()
{
  ...
  if (file)
  {
    ...
    file.write(redisIP, 15);
    file.write(redisPW, 15);
    file.write(redisModule, 3);
...
void delConfigData()
{
  ...
  LOGERROR(F("delWiFiCfgFile "));

  
}

void waitforresetwifi()
{ int i=0;
  Serial.println(F("Waiting for hold on reset button-> reconfigure wifi"));
  while (i<20){
    if(digitalRead(RESETWIFI)==0){
     initialConfig=true;
     Serial.println(F("Wifi reconfig called, wait..."));
    }
    delay(300);
    i++;
    Serial.print(F("."));
...
void resetwifi()
{  
  Serial.println(F("Remove config wifi credentials and enter to Captive portal"));
    delay(1000);
...
void setupwifi()
{
//wait reset button for reconfigure wifi
waitforresetwifi();
...
#endif
//custom parameter for redis server IP
ESP_WMParameter p_redisIP("RedisIP", "REDIS server IP", redisIP, 15);
ESP_WMParameter p_redisPW("RedisPW", "REDIS password", redisPW, 15);
ESP_WMParameter p_redisModule("RedisModule", "REDIS Module(1-99)", redisModule, 3);
ESP_wifiManager.addParameter(&p_redisIP);
ESP_wifiManager.addParameter(&p_redisPW);
ESP_wifiManager.addParameter(&p_redisModule);
  ...
  String AP_SSID = "Aerin_" + chipID ;
  String AP_PASS = "AerinSistemas";
  ...
  if (initialConfig)
  {
    ...
    //////
    //transform to global vars 
    strcpy(redisIP,p_redisIP.getValue());
    strcpy(redisPW,p_redisPW.getValue());
    strcpy(redisModule,p_redisModule.getValue());
  ...
  //for reset and clear credential file
